...
\author{
  \IEEEauthorblockN{Connor Nealis \ Rianne Joseph \ Muhammad Abdullah \ Tayat Angkasuwapala}
  \IEEEauthorblockA{
...
as shown in Fig.~\ref{fig:loopback_schem} and Fig.~\ref{fig:pcb} (Appendix). Total
...
\begin{align}
  M &= \bigl(179 - 10(l_0{-}\texttt{'A'}) 
           - (l_2{-}\texttt{'0'})\bigr)\cdot 180 \notag \\
...
recovered correctly. Terminal screenshots are shown in Fig.~\ref{fig:decode1} and
Fig.~\ref{fig:decode2} (Appendix).

% fig:decode1, fig:decode2 moved to appendix
...
First, we calculate the $V_\text{peak}$ required to drive 100~mW into 50~$\Omega$.
From $P = V_\text{RMS}^2 / R$:
%
\begin{equation}
  V_\text{RMS} = \sqrt{P \cdot R} = \sqrt{0.1 \times 50} = 2.236\,\text{V}
  \label{eq:vrms}
\end{equation}
%
...
We first select C33. Standard values of temperature-compensated ceramic capacitors
used in RF applications are 220~pF, 330~pF, 470~pF, and 680~pF. We choose 470~pF.
...
\end{equation}
%
Substituting $f_0 = 7\,\text{MHz}$ and $C = 470\,\text{pF}$ gives $L = 1.09\,\mu\text{H}$.
The nearest standard value is $1\,\mu\text{H}$; hence L5~=~$1\,\mu\text{H}$.

L4 provides DC current to the MOSFET drain and blocks RF from entering the power supply.
At 7~MHz we require $X_L \gg 50\,\Omega$, where:
%
\begin{equation}
  X_L = 2\pi f_0 L
  \label{eq:xl}
\end{equation}
%
Choosing the standard value $L = 10\,\mu\text{H}$ gives $X_L = 440\,\Omega \gg 50\,\Omega$,
so the supply path appears as a near open-circuit to RF.

The gate series resistor R22 limits instantaneous gate charging current and suppresses
ringing. Its exact value is not critical; typical values range from $10\,\Omega$ to
$100\,\Omega$. The standard value $47\,\Omega$ is chosen.

The gate pull-down resistor R23 ensures the MOSFET stays off at power-up and discharges
the gate capacitance. With the BS170 input capacitance $C_\text{iss} \approx 25\,\text{pF}$,
choosing $R23 = 100\,\text{k}\Omega$ gives a time constant
$\tau = RC = 2.5\,\mu\text{s}$, which is sufficiently fast.
...
The completed schematic is shown in Fig.~\ref{fig:pcb_schem} (Appendix).
...
The completed PCB layout is shown in Fig.~\ref{fig:pcb_layout} (Appendix).
...
\bibitem{e14}
Element14, ``Component Search and Supply,'' [Online].
Available: \url{https://au.element14.com/}
...
\section{Hardware and Decode Screenshots}
...
\section{KiCad Schematic and PCB Layout}
...
Full source: \texttt{CODE.zip} --- \texttt{wspr\_encode.c}
